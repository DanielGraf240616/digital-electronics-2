...
// WRITE YOUR CODE HERE
    /** @brief Stop timer, prescaler 000 --> STOP */
#define TIM0_STOP TCC0RB &= ~((1<<CS02) | (1<<CS01) | (1<<CS00));
...
#define TIM0_OVF_1MS TCCR1B &= ~(1<<CS02); TCCR0B |= (1<<CS01) | (1<<CS00);
 ...
 */
// WRITE YOUR CODE HERE
#define TIM2_STOP TCCR2A &= ~((1<<CS22) | (1<<CS21) | (1<<CS20));

    /** @brief Set overflow 16us, prescaler 001 --> 1 */
#define TIM2_OVF_16U TCCR2A &= ~((1<<CS22) | (1<<CS21)); TCCR2A |= (1<<CS20);

    /** @brief Set overflow 128us, prescaler 010 --> 8 */
#define TIM2_OVF_128u  TCCR2A &= ~((1<<CS22) | (1<<CS20)); TCCR2A |= (1<<CS21);

     /** @brief Set overflow 51ms, prescaler 011--> 32 */ 
#define TIM2_OVF_51MS TCCR2A &= ~(1<<CS22); TCCR2A |= (1<<CS21) | (1<<CS20);

    /** @brief Set overflow 1.024ms, prescaler 100 --> 64 */ 
#define TIM2_OVF_1MS  TCCR2A &= ~((1<<CS21) | (1<<CS20)); TCCR2A |= (1<<CS22);

    /** @brief Set overflow 2.048ms, prescaler 101 --> 128 */   
#define TIM2_OVF_2MS  TCCR2A &= ~(1<<CS21); TCCR2A |= (1<<CS22) | (1<<CS20);        

    /** @brief Set overflow 4ms, prescaler 110 --> 256 */   
 #define TIM2_OVF_4MS  TCCR2A &= ~ (1<<CS20); TCCR2A |= (1<<CS22)| (1<<CS21);   

    /** @brief Set overflow 16ms, prescaler // 111 --> 1024 */  
#define TIM2_OVF_16MS  TCCR2A |= (1<<CS22) | (1<<CS21) | (1<<CS20);
